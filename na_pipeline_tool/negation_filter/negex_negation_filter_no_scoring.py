...
class NegexNegationFilterNoScoringModule(helper_classes.Module):
        ...
        for _params in param_list:
            ...
            for _k, _v in self._lexicon_map[cat].items():
                ...
                for sent in sents:
                    ...
                    if num_neg + num_affirm > 1:
                        break
            
                    negation_info_dict[id][_k]['sentences'].append(sent)
                    negation_info_dict[id][_k]['occurences'].append(affirmed_tags)
                    negation_info_dict[id][_k]['negated_occurences'].append(negated_tags)
            
                if num_neg == 1 and num_affirm == 0:
                    negation_info_dict[id][_k]['negated'] = True
                elif num_neg > 0 or num_affirm > 0:
                    negation_info_dict[id][_k]['negated'] = False
                else:
    ...
    def _check_note_negations(self):
        ...
        if self._debug_check:
            note_pos_df = note_pos_df[note_pos_df['ROW_ID'] == 2443]
            ...
            for _id, _cat_dict in _entry.items():
                ...
                for _cat in self._lexicon_map['positive'].keys():
                    ...
                    if not _negated:
                        ...
                        positive_negations = 1
                      
                    lis.append(positive_terms - positive_negations)
                    lis.append(positive_terms)

                    lis.append('\n'.join(_cat_dict[_cat]['sentences']))
                    lis.append('\n'.join([str(_) for _ in _cat_dict[_cat]['occurences']]))
                    lis.append('\n'.join([str(_) for _ in _cat_dict[_cat]['negated_occurences']]))
    ...
    def register_argparser_object(cls, subparser_instance):
        subparser_instance.add_parser(cls.module_name(), help="Filters the pre-labeled notes and removes negated instances from the final note dataset by using the Negex utility - W/O Scoring!.")
